...
########################################################################
class SpreadRBIStrategy(CtaTemplate):
    """二腿合约价差计算"""
    className = 'SpreadRBIStrategy'
    author = u'clw@itg'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(SpreadRBIStrategy, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        self.writeCtaLog(u'螺纹铁矿比值套利初始化')
        self.pending = []
        self.completed = []
        self.qryCount = 0
        self.trading = False
    ...
    def onStart(self):
        """启动策略（必须由用户继承实现）"""
        self.writeCtaLog(u'螺纹铁矿比值套利合约策略启动')
        self.trading = True
        self.timerCount = 0
        # self.lastOrderCompleted = datetime.datetime.now() - datetime.timedelta(days=3)
    ...
    def onStop(self):
        ...
        self.writeCtaLog(u'螺纹铁矿比值套利策略停止')
        self.timerCount = 0
        self.putEvent()
    ...
    def calcPrice(self, offset, tick, pair, sendOrderFunc):
        ...
        if self.trading and self.checkIfTrading() and pair[0]['price'] * pair[1]['price'] > 0 \
